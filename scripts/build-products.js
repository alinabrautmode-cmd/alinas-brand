...
const homeContentPath = path.join(rootDir, 'content', 'pages', 'home.json');
const homeOutputPath = path.join(rootDir, 'assets', 'js', 'home.js');
...
function normalizeHomeImage(src) {
  if (!src) return '';
  return String(src).trim();
}

function normalizeHomeContent(raw) {
  const hero = raw && raw.hero ? raw.hero : {};
  ...
  function normalizeCategory(key) {
    ...
    const fallback = defaultHome.categories[key];
    return {
      image: normalizeHomeImage(input.image) || fallback.image,
  ...
  return {
    hero: {
      ...
      subtitle: String(hero.subtitle || defaultHome.hero.subtitle).trim(),
      buttonText: String(hero.buttonText || defaultHome.hero.buttonText).trim(),
      buttonUrl: String(hero.buttonUrl || defaultHome.hero.buttonUrl).trim(),
      kicker: String(hero.kicker || defaultHome.hero.kicker).trim()
    },
    categories: {
      dresses: normalizeCategory('dresses'),
      suits: normalizeCategory('suits'),
      jumpsuits: normalizeCategory('jumpsuits')
    }
  };
}

function buildHomeContent() {
  const rawHome = fs.existsSync(homeContentPath) ? readJson(homeContentPath) : {};
...
window.ALINAS_BRAND_HOME = ${JSON.stringify(home, null, 2)};
`;
  fs.writeFileSync(homeOutputPath, output);
  console.log(`Generated ${path.relative(rootDir, homeOutputPath)} from ${path.relative(rootDir, homeContentPath)}.`);
}
